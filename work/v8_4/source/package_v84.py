"""Package only the validated V8.4 prototype; no printer operations."""
from pathlib import Path
import hashlib,json,shutil,zipfile
ROOT=Path(__file__).resolve().parent.parent;PROJECT=ROOT.parents[1];OUT=PROJECT/'dist/v8_4'
...
def main():
 ...
 for name in ['faceplate','faceplate_NO_SPRING_ONLY','rear_tray','battery_cover']:
  shutil.copy2(ROOT/'parts'/(name+'.3mf'),OUT/('CatBadge-v8.4-'+name+'.3mf'))
 ...
 archive=OUT/'CatBadge-v8.4-Handheld-Case.zip'
 with zipfile.ZipFile(archive,'w',zipfile.ZIP_DEFLATED) as z:
  for p in sorted(files):z.write(p,'CatBadge-v8.4/'+str(p.relative_to(ROOT)))
 with zipfile.ZipFile(archive) as z:
  assert z.testzip() is None
  for p in files:assert hashlib.sha256(z.read('CatBadge-v8.4/'+str(p.relative_to(ROOT)))).hexdigest()==sha(p)
