...
def main():
 ...
 for path in ROOT.glob('*.3mf'):
  ...
  req(doc.get('unit')=='millimeter','units');req('v8.7' in doc.find("m:metadata[@name='Title']",NS).text,'title')
  objs=doc.findall('m:resources/m:object',NS);expected=(4 if 'Snap_Only' in path.name else 3 if 'Comfort_Only' in path.name else 16) if 'PRINT_FIRST' in path.name else 9;req(len(objs)==len(set(o.get('name') for o in objs))==expected,'kit count')
 ...
 for i,snap in enumerate(g.SNAPS):
  access=g.solid([(-1.15,-.15,g.slab(snap,5.2,7.8,1.7,4.5))])
 ...
 from validate_closed_back import check_revision
 report['checks']['v87_closed_back_and_revised_joints']=check_revision(lid,alt,rear,cover)
